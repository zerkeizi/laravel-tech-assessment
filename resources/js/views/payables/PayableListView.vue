...
async function load(page = 1) {
    const { data } = await payablesApi.index({ ...filters, page });
    rows.value = data.data;
    meta.value = { current_page: data.current_page, last_page: data.last_page, total: data.total };
}

async function pay(payable) {
    error.value = '';
    try {
        await payablesApi.pay(payable.id);
        await load(meta.value?.current_page ?? 1);
    } catch (e) {
        error.value = e.response?.data?.message ?? 'Não foi possível registrar o pagamento.';
    }
}

async function remove(payable) {
    if (!confirm(`Excluir "${payable.description}"?`)) return;
    await payablesApi.destroy(payable.id);
    await load(meta.value?.current_page ?? 1);
}
...
<template>
    <div>
        <div class="header">
            ...
            <RouterLink to="/payables/create" class="btn">Nova</RouterLink>
        </div>
